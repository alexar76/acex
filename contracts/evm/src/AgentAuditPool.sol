...
contract AgentAuditPool is IAgentAuditPool, ReentrancyGuard, Ownable2Step, Pausable {
    ...
    mapping(bytes32 => uint256) public compensationPerNoteE6;
    mapping(bytes32 => uint256) public defaultCompensationPool;
    ...
    function onListingApproved(bytes32 listingId) external onlyRegistry {
        ...
        baselinePriceE18[listingId] = _currentSharePriceE18(listingId);
        lastObservedAt[listingId] = uint64(block.timestamp);
        lastObservedPriceE18[listingId] = baselinePriceE18[listingId];
    }
    ...
    /// @notice One-shot baseline capture after AMM pool exists (must run before triggerDefault).
    function captureBaseline(bytes32 listingId) external whenNotPaused {
        ...
        if (price == 0) revert DefaultConditionsNotMet();

        baselinePriceE18[listingId] = price;
        lastObservedAt[listingId] = uint64(block.timestamp);
        lastObservedPriceE18[listingId] = price;
        emit SharePriceObserved(listingId, price, price);
    }
    ...
    function triggerDefault(bytes32 listingId) external nonReentrant whenNotPaused {
        ...
        if (baseline == 0) revert BaselineNotSet();
        ...
        whenNotPaused
    {
        ...
        uint256 perNote = compensationPerNoteE6[listingId];
        if (perNote == 0) revert NoCompensation();
        ...
        if (newTotal > defaultNoteSupply[listingId]) revert ExceedsClaimableNotes();

        uint256 payout = (noteAmount * perNote) / 1e18;
