...
contract AgentLendingPool is ReentrancyGuard, Ownable, Pausable {
    ...
    ///      MINIMUM_LIQUIDITY is locked forever; making it 1e6 (1 USDC) means a
    ///      donation must dwarf 1e6 locked shares to inflate the price, so the
    ///      attack costs far more than it can steal. MIN_FIRST_DEPOSIT must exceed
    ///      MINIMUM_LIQUIDITY so the first real depositor still receives shares.
    uint256 public constant MIN_FIRST_DEPOSIT = 2e6; // 2 USDC (6 decimals)
    uint256 public constant MINIMUM_LIQUIDITY = 1e6; // 1 USDC locked forever
